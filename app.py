...
def predict_yield(state_name, district_name, crop, area_ha, weather_data, soil_ph):
    """Predict crop yield - FIXED for XGBoost categorical handling"""
    new_data_dict = {
    ...
    # FIXED: Apply label encoding properly for XGBoost
    for col in cat_cols:
        if col in new_df.columns:
            val = str(new_df[col].iloc[0])
            if val not in label_encoders[col].classes_:
                # Use first class for unseen values
                print(f"Warning: '{val}' not seen in training for {col}, using '{label_encoders[col].classes_[0]}'")
                new_df.loc[0, col] = label_encoders[col].transform([label_encoders[col].classes_[0]])[0]
            else:
                new_df.loc[0, col] = label_encoders[col].transform([val])[0]
    
    # FIXED: Ensure categorical columns are converted to proper numeric types
    for col in cat_cols:
        if col in new_df.columns:
            new_df[col] = new_df[col].astype('int32')  # Convert to numeric type

    # Scale numerical values
    new_df[num_cols] = scaler.transform(new_df[num_cols])
    
    # FIXED: Ensure all columns are numeric before prediction
                ...
                if lat and lon:
                    ...
                    if weather_data:
                        try:
                            ...
                            st.error("Please check if your model files are properly trained for XGBoost")
                    else:
